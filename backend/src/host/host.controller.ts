...
export class HostController {
    ...
    // create listing
    @Post('listing')
    async createListing(@Body() updateData: UpdateListingDto) {
        return await this.hostService.createListing(updateData)
    }

    // update listing
    @Put('listing/:listingId')
    async updateListing(@Param('listingId') listingId: string, @Body() updateData: UpdateListingDto) {
    ...
    //get all listings
    @Get('listings/:hostId')
    async getListings(@Param('hostId') hostId: string,) {
        return await this.hostService.getListings(hostId)
    }

    // get reservations
    @Get('reservations:hostId')
    async getReservations(@Param('hostId') hostId: string,) {
