...
export class PaymentService {
    ...
    async createPaymentIntent(dto: PaymentIntentDto) {
        try {
            ...
            if(!host) throw new BadRequestException(`Invalid listing found. Contact support at ${process.env.SUPPORT_EMAIL}`)
            if(!host.stripe_account) throw new BadRequestException(`Host is required to complete extra steps. Contact support at ${process.env.SUPPORT_EMAIL}`)
    ...
    async getCheckoutDetails(dto: GetCheckoutDetailsDto) {
        try {
            ...
            const { totalPrice, differenceInDays } = this.calculatePriceByDates(listing.price, new Date(startDate), new Date(endDate))
