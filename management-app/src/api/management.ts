...
export const onboardManagement = async (userId: string, dto: z.infer<typeof OnboardManagementValidation>) => {
    const {password, confirmPassword, oldPassword, ...restDto} = dto
    const response = await client.post('/onboard-management', {userId, ...restDto})
    return response.data
}
export const onboardBuilding = async (userId: string, dto: z.infer<typeof OnboardBuildingValidation>) => {
