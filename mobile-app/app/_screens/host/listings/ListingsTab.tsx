import { View, Text, SafeAreaView, ScrollView, StyleSheet, FlatList, TouchableOpacity, Linking, Alert } from "react-native";
...
const ListingsTab = () => {
  ...
  const { data: listings, isFetching: listingLoading, refetch } = useGetHostListings(user?.id ?? "");
  ...
  useLayoutEffect(() => {
    if (user?.stripe_account && !(user.stripe_account as any)["requirements"]["disabled_reason"]) {
  ...
  if ((user.stripe_account as any)["requirements"]["disabled_reason"]) {
    return (
      <View >
        <TouchableOpacity style={styles.actionLinkBtn} onPress={handleFinishMoreActions}>
          <Text style={styles.actionLinkBtnText}>You need to finish more actions before you can begin listing. Click to finish</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView style={{ padding: 20 }}>
        ...
        {listings.map((listing) => (
          <TouchableOpacity key={listing.id} style={styles.listingContainer} onPress={() => navigation.navigate("ListingDetails", { listing })}>
